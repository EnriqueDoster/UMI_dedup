...
if __name__ == "__main__":
	...
	io.TextIOWrapper(io.BufferedReader(gzip.open(args.reverse, 'rb'))) as r:
		for (ftitle, fread, fqual), (rtitle, rread, rqual) in zip(FastqGeneralIterator(f), FastqGeneralIterator(r)):
			...
			try:
				fumi[fread[0:10]][fread[10:50]] += 1
			except KeyError:
				try:
					fumi[fread[0:10]].setdefault(fread[10:50], 1)
				except KeyError:
					fumi.setdefault(fread[0:10], {fread[10:50]: 1})
			try:
				rumi[rread[0:10]][rread[10:50]] += 1
			except KeyError:
				try:
					rumi[rread[0:10]].setdefault(rread[10:50], 1)
				except KeyError:
					rumi.setdefault(rread[0:10], {rread[10:50]: 1})

	samplename = args.forward.split('/')[-1].split('_R1')[0]
	print("Working on sample",samplename)
	with gzip.open(args.output + '/' + samplename + '_50dedup_R1.fastq.gz', 'wb') as fout, \
	gzip.open(args.output + '/' + samplename + '_50dedup_R2.fastq.gz', 'wb') as rout:
